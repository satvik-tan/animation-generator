...
export default function Landing() {
  return (
    <div className="min-h-screen bg-white">
      ...
      <nav className="fixed top-0 z-50 w-full border-b border-gray-200 bg-white">
        <div className="max-w-6xl mx-auto flex h-16 items-center justify-between px-4 sm:px-6">
          <div className="flex items-center gap-2">
            ...
            <span className="text-base sm:text-lg font-semibold text-gray-900">Manimation</span>
          </div>
          <Link
            to="/app"
      ...
      <section className="flex min-h-screen items-center justify-center pt-16 px-4 sm:px-6">
        <div className="max-w-3xl text-center">
          ...
          <div className="mt-8 flex flex-col items-center gap-3 sm:flex-row sm:justify-center">
            <Link
              to="/app"
              className="px-6 py-3 text-sm font-semibold text-white bg-gray-900 rounded hover:bg-gray-800 transition-colors"
            >
              Open App
            </Link>
          </div>
      ...
      <section className="py-16 sm:py-24 px-4 sm:px-6 bg-gray-50">
        <div className="max-w-6xl mx-auto">
          <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-4">
            {features.map((f) => (
              <div key={f.title} className="p-6 border border-gray-200 rounded bg-white hover:border-gray-300 transition-colors">
      ...
      {/* How it works */}
      <section className="border-t border-gray-200 py-16 sm:py-24 px-4 sm:px-6">
          ...
          <div className="mt-16 grid gap-12 sm:grid-cols-3">
            {[
              ...
              <div key={s.step} className="flex flex-col items-center">
                <span className="text-5xl font-bold text-gray-200">{s.step}</span>
                <h3 className="mt-4 text-xl font-semibold text-gray-900">{s.title}</h3>
                <p className="mt-2 max-w-xs text-sm text-gray-600">{s.text}</p>
              </div>
            ))}
          </div>
        </div>
      ...
      <footer className="border-t border-gray-200 py-12 px-4 sm:px-6">
        <div className="max-w-6xl mx-auto flex flex-col items-center gap-4 text-center text-sm text-gray-600">
